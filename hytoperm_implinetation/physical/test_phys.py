...
class World:
    ...
    def __init__(self) -> None:
        ...
        # Define trial folder location
        base_path = os.path.expanduser('~/Documents/hytoperm_implinetation/physical/Trials')
        
        # Auto-increment trial number to avoid overwriting
        while os.path.isdir(os.path.join(base_path, f'trial{num}')):
            num += 1

        trial_path = os.path.join(base_path, f'trial{num}')
        os.makedirs(trial_path, exist_ok=True)

        count = 0

        # Save full cycle position and control trajectories
        with open(os.path.join(trial_path, f'cycleInfo{num}_total_points.json'), "w") as final:
            json.dump(ex.agent()._cycle.pTrajectory.x.tolist(), final)

        with open(os.path.join(trial_path, f'cycleInfo{num}_total_cntrls.json'), "w") as final:
            json.dump(ex.agent()._cycle.uTrajectory.x.tolist(), final)
    

        # Store segment-by-segment trajectory data
        for ts in ex.agent()._cycle._trajectorySegments:
            ...
            with open(os.path.join(trial_path, f'cycleInfo{num}_{count}_points.json'), "w") as final:
                json.dump(ptraj.tolist(), final)
            with open(os.path.join(trial_path, f'cycleInfo{num}_{count}_cntrls.json'), "w") as final:
                json.dump(utraj.tolist(), final)
            with open(os.path.join(trial_path, f'cycleInfo{num}_{count}_dynams.json'), "w") as final:
        ...
        pickle.dump(fig, open(os.path.join(trial_path, f'Worldplot{num}.pickle'), 'wb'))
        plt.show()
