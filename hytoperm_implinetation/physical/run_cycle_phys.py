...
def loadPoints(num,tot=None):
    ...
    base_path = os.path.join('Trials', f'trial{num}')

    if tot is None:
    # Count only complete sets of files
        files = glob.glob(os.path.join(base_path, f'cycleInfo{num}_*_points.json'))
        complete_counts = [
            int(f.split('_')[-2])
            for f in files
    ...
    for count in range(tot):
        ...
        f = open(os.path.join(base_path, f'cycleInfo{num}_{count}_points.json'), 'r')
        points_dict[count] = json.loads(f.readline())

        # save the trajectory controls
        f = open(os.path.join(base_path, f'cycleInfo{num}_{count}_cntrls.json'), 'r')
        uts_dict[count] = json.loads(f.readline())

        
        # Get the hybrid dynamics of the region
        f = open(os.path.join(base_path, f'cycleInfo{num}_{count}_dynams.json'), 'r')
        
        '''        
        hds_dict[count] = np.array(json.loads(f.readline()))
        #reshape the hybrid dynamics to a 2D vector
        hds_dict[count].reshape((2,1))
        According to GPT, the lines above don't do anything. Replaced with the line below
        '''
    ...
    f = open(os.path.join(base_path, f'cycleInfo{num}_total_points.json'), 'r')
    pts_2 = np.array(json.loads(f.readline()))
    f = open(os.path.join(base_path, f'cycleInfo{num}_total_cntrls.json'), 'r')
